...
(
  ...
  , sum(case when extract(epoch from io.charttime - iosum.charttime)/3600.0 <= 5
      then iosum.VALUE
    else null end) as urineoutput_6hr
  , sum(case when extract(epoch from io.charttime - iosum.charttime)/3600.0 <= 11
      then iosum.VALUE
  ...
  , MIN(case when io.charttime <= (iosum.charttime + INTERVAL '5 hours')
      then iosum.charttime
    else null end)
    AS starttime_6hr
  , MIN(case when io.charttime <= (iosum.charttime + INTERVAL '11 hours')
      then iosum.charttime
    else null end)
    AS starttime_12hr
...
(
  ...
  , ROUND((extract(epoch from charttime - starttime_6hr)/3600.0)::numeric, 4)::double precision + 1 AS uo_tm_6hr
  , ROUND((extract(epoch from charttime - starttime_12hr)/3600.0)::numeric, 4)::double precision + 1 AS uo_tm_12hr
  , ROUND((extract(epoch from charttime - starttime_24hr)/3600.0)::numeric, 4)::double precision + 1 AS uo_tm_24hr
  from ur_stg
)
insert into {mimic_derived}.kdigo_uo
select
  ur.icustay_id
, ur.charttime
, wd.weight
...
, ur.urineoutput_12hr
, ur.urineoutput_24hr
, ROUND((ur.urineoutput_6hr/wd.weight/uo_tm_6hr)::numeric, 4)::double precision AS uo_rt_6hr
, ROUND((ur.urineoutput_12hr/wd.weight/uo_tm_12hr)::numeric, 4)::double precision AS uo_rt_12hr
, ROUND((ur.urineoutput_24hr/wd.weight/uo_tm_24hr)::numeric, 4)::double precision AS uo_rt_24hr
